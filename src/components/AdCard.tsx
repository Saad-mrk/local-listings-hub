...
const AdCard = ({ id, title, price, city, image, date, seller = "Vendeur" }: AdCardProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="group">
      <Link to={`/ad/${id}`} className="block">
        <div className="bg-card rounded-2xl overflow-hidden border border-border shadow-card hover:shadow-card-hover transition-shadow duration-300">
          <div className="relative aspect-[4/3] overflow-hidden bg-muted">
            ...
            <button onClick={handleToggleLiked} className="absolute top-3 right-3 w-9 h-9 rounded-full bg-background/80 backdrop-blur-sm flex items-center justify-center hover:bg-background transition-colors">
              <Heart className={`h-5 w-5 transition-colors ${liked ? "fill-red-500 text-red-500" : "text-muted-foreground"}`} />
            </button>
          </div>
          <div className="p-4">
            <p className="text-lg font-heading font-bold text-primary">{price.toLocaleString()} DH</p>
            <h3 className="text-sm font-medium text-foreground mt-1 line-clamp-2 leading-snug">{title}</h3>
            <button onClick={handleAddToCart} className="w-full mt-3 bg-add-to-cart text-white font-medium py-2 px-3 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-105">
