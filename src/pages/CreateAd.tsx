      ...
      <div className="container max-w-2xl py-8">
        ...
        <div className="bg-card rounded-2xl border border-border p-6 shadow-card space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {images.map((img, i) => (
                <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-muted">
                  ...
                  <button onClick={() => removeImage(i)} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-foreground/70 flex items-center justify-center"><X className="h-4 w-4 text-background" /></button>
                </div>
              ))}
              {images.length < 6 && (
                <button onClick={() => fileRef.current?.click()} onDrop={(e) => { e.preventDefault(); handleFiles(e.dataTransfer.files); }} onDragOver={(e) => e.preventDefault()}
                  ...
                  <Upload className="h-6 w-6" /><span className="text-xs">{t("add_photo")}</span>
                </button>
              )}
            </div>
          ...
          <div>
            ...
            <input type="text" placeholder={t("title_placeholder")} className="w-full h-11 px-4 rounded-xl bg-muted/50 border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
          </div>
          <div>
            <label className="text-sm font-medium mb-1.5 block">{t("description_label")}</label>
            <textarea rows={4} placeholder={t("description_placeholder")} className="w-full px-4 py-3 rounded-xl bg-muted/50 border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
          </div>
          ...
          <Button className="w-full h-12 bg-primary hover:bg-primary-hover text-primary-foreground rounded-xl font-semibold text-base">{t("publish_ad")}</Button>
        </div>
      </div>
    </motion.div>
  );
